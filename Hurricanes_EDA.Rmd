...
(Converted from XML to CSV anf used below)
...
# Read Annual Tropical Storm Data from 1958 - 2021, extract years and corresponding number of tropical storms 
```{r}
tropical_storm_data <- read.csv('datasets/tropical_storm_data.csv')
years = unique(tropical_storm_data[["Year"]])
freq = c()
for(year in years) {
  freq <- append(freq, nrow(subset(tropical_storm_data, Year == year)))
}
max_freq = max(freq)
```

# Plot a barplot with Year on X-Axis and Storm Frequency on Y-Axis
```{r}
barplot(
  freq,
  names.arg = years,
  main = 'All Tropical Storms by Year (Pacific & Atlantic)',
  xlab = 'Year',
  ylab = 'Frequency',
  las = 2
)
...
```{r}
plot(
  0:max(max_freq, length(freq) - 1),
  freq,
  main = 'Poisson Distribution over all Tropical Storm (Pacific & Atlantic) Frequencies',
  xlab = 'Tropical Storm Frequency in a year',
  ylab = 'Count of Years',
  ylim = c(0,125)
)
lines(0:max(max_freq, length(freq) - 1),
      dpois(0:max(max_freq, length(freq) - 1), lambda) * sum(freq),
      lwd = 2)
grid()
```
...
qqplot(
  dpois(0:max(max_freq, length(freq) - 1), lambda) * sum(freq),
  freq,
  main = 'Poisson Q-Q Plot for all Tropical Storms (Pacific & Atlantic)',
  xlab = 'Theoretical Quantiles',
  ylab = 'Sample Quantiles'
)
abline(0, 1)
